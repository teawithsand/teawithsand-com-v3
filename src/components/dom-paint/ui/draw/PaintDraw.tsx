...
import PaintDrawDisplay, {
	DrawUIEvent,
} from "@app/components/dom-paint/ui/draw/PaintDrawDisplay"
...
export default (props: {
	...
	const { onMutationsChanged, onGlobalUIStateChanged } = props
	...
	const uiStateManager = useMemo(
		() => new GlobalUIStateManagerImpl(initialGlobalUIState),
		[]
	)
	const uiSceneManager = useMemo(
		() => new UISceneManagerImpl(initialMutations),
		[]
	)
	...
	const [toolProps, setToolProps] = useState<{ [key: string]: any } | null>(
		null
	)
	...
	const uncommitedMutations = useStickySubscribable(
		uiSceneManager.uncommitedMutations
	)
