...
export const parseColor = (input: string): Color => {
	if (input.substring(0, 1) == "#") {
		...
		return [
			Math.round(parseInt(input.substring(1, (1) + columnLength), 16) * fact),
			Math.round(
				parseInt(input.substring(1 + columnLength, (1 + columnLength) + columnLength), 16) * fact
			),
			Math.round(
				parseInt(input.substring(1 + 2 * columnLength, (1 + 2 * columnLength) + columnLength), 16) *
					fact
			),
		]
	} else {
