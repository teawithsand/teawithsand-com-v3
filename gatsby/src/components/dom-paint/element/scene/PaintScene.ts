...
import PaintLayer, {
	newEmptyPaintLayerData,
} from "@app/components/dom-paint/element/scene/PaintLayer"
...
export const paintSceneEventSourcingAdapter: EventSourcingAdapter<
	PaintScene,
	PaintSceneMutation
> = {
...
export default class PaintScene
	implements PaintSceneQuery, RenderHashable<PaintSceneData>
{
	...
	getElementWithLocator = (
		l: PaintSceneElementLocator,
		ignoreOutdated = true
	): PaintElement | null => {
	...
	updateWithMutation = (m: PaintSceneMutation) => {
		this.updateData(data => {
			if (m.type === "push-layer") {
				...
				this.ensureLayerNoNotify(m.index).updateData(layerData => {
				...
				this.ensureLayerNoNotify(m.layerIndex).updateData(layerData => {
					insertAt(
						layerData.elements,
						m.beforeElementIndex ?? layerData.elements.length,
						...m.elements
					)
					return layerData
				})
