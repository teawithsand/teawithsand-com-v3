...
export default (props: {
    ...
    return <div className={classnames(animation["app-tea"], props.className)}>
        {
            range(maxColumn)
                .map((i) => <div
                    key={i}
                    className={
                        classnames(
                            animation[`app-tea__sand`],
                            animation[`app-tea__sand--flow-${randomSands[0][i]}`],
                            animation[`app-tea__sand--col-${i + 1}`]
                        )}
                ></div>)
        }
        {
            range(maxColumn)
                .map((i) => <div
                    ...
                    className={classnames(
                        animation[`app-tea__sand`],
                        animation[`app-tea__sand--flow-${randomSands[1][i]}`],
                        animation[`app-tea__sand--col-${i + 1}`]
                    )}
                ></div>)
        }
        {
            range(auxSandCount)
                .map((i) => <div
                    ...
                    className={classnames(
                        animation[`app-tea__sand`],
                        animation[`app-tea__sand--flow-${randomSands[2][i]}`],
                        animation[`app-tea__sand--col-${randomColumns[i]}`]
                    )}
                ></div>)
        }
    </div >
