...
import UISceneManager from "@app/Component/DOMPaint/ui/scene/UISceneManager";
import { StickySubscribable } from "@app/util/lang/bus/stateSubscribe";
import StickyEventBus, { DefaultStickyEventBus } from "@app/util/lang/bus/StickyEventBus";
import { EventSourcing, InMemoryEventSourcing } from "@app/util/lang/eventSourcing";

export default class UISceneManagerImpl implements UISceneManager {
    private readonly innerSceneEventSourcing: EventSourcing<PaintScene, PaintSceneMutation>
    private ephemeralStack: PaintSceneMutation[] = []
    private innerUncommitedMutations: StickyEventBus<PaintSceneMutation[]> = new DefaultStickyEventBus([])

    constructor(initialMutationsStack: PaintSceneMutation[]) {
        this.innerSceneEventSourcing = new InMemoryEventSourcing<PaintScene, PaintSceneMutation>(
    ...
    popFromEphemeralStack = (): boolean => {
        if (this.ephemeralStack.length > 0) {
            ...
            if (!e)
                throw new Error("Expected mutation to be there")
