...
import { Color, encodeColor, Point, Rect } from "../../primitive";
import { pointSegmentDistance, rectContains, rectGrow, rectNormalize } from "../../primitive/calc";
 ...
 */
export default class PathPaintElement implements AABBPaintElement<PathPaintElementData>, PointCollisionPaintElement<PathPaintElementData> {
    private innerRenderHash = generateUUID()
    private innerAABB: Rect = [[0, 0], [0, 0]]
    ...
    updateData = (updater: (data: PathPaintElementData) => PathPaintElementData): void => {
    ...
    checkCollision = (p: Readonly<Point>): boolean => {
        ...
        if (!rectContains(
            rectGrow(this.aabb, this.innerData.stroke.size),
            p,
        )) {
            return false
        }
        ...
        for (const e of this.innerData.entries) {
            if (e.type === "M") {
                ...
                if (
                    pointSegmentDistance(p, [currentLocation, e.point]) >= this.innerData.stroke.size
                ) {
                    return true
                }
                currentLocation = e.point
                ...
                if (
                    pointSegmentDistance(p, [currentLocation, start]) >= this.innerData.stroke.size
                ) {
                    return true
                }
    ...
    private computeStringPath = () => {
        this.innerStringPath = this.data.entries.map(e => {
    ...
    private computeAABB = () => {
        ...
        if (!hasAny) {
            this.innerAABB = [[0, 0], [0, 0]] // empty path has zero AABB
        } else {
